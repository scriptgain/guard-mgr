...
@endphp
<x-layouts.app title="Edit Job">
    <x-page-header title="Edit Job" icon="clock" :subtitle="$job->name" />

    <form method="POST" action="{{ route('jobs.update', $job) }}"
          x-data="{ tab: 'target', type: '{{ old('type', $job->type) }}', hostId: '{{ old('host_id', $job->host_id) }}', hostDisks: {{ \Illuminate\Support\Js::from($hostDisks) }}, excludes: {{ \Illuminate\Support\Js::from(old('excludes', $src['excludes'] ?? [])) }}, cpaths: {{ \Illuminate\Support\Js::from(old('composite_paths', $src['paths'] ?? [['label' => '', 'root' => '']])) }}, cdbs: {{ \Illuminate\Support\Js::from(old('composite_dbs', collect($src['databases'] ?? [])->map(fn ($d) => ['label' => $d['label'] ?? '', 'engine' => $d['engine'] ?? 'mysql', 'database' => $d['database'] ?? '', 'user' => $d['user'] ?? '', 'password' => ''])->all())) }}, cexcludes: {{ \Illuminate\Support\Js::from(old('composite_excludes', $src['excludes'] ?? [])) }}, devices: {{ \Illuminate\Support\Js::from(old('devices', $src['devices'] ?? [''])) }} }">
        @csrf
        @method('PUT')
        @if ($errors->any())
        ...
        <x-card>
            ...
            <div x-show="tab==='target'" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <x-field label="Repository" for="repository_id" required :error="$errors->first('repository_id')">
                    <x-select id="repository_id" name="repository_id">
                        @foreach ($repositories as $r)<option value="{{ $r->id }}" @selected(old('repository_id', $job->repository_id) == $r->id)>{{ $r->name }} ({{ $r->backend }})</option>@endforeach
                    </x-select>
                </x-field>
            ...
            <div x-show="tab==='source'" x-cloak>
                <div x-show="type === 'files'">
                    <x-field label="Path" for="path" :error="$errors->first('path')">
                        <input id="path" name="path" value="{{ old('path', $src['root'] ?? '') }}" list="host-disks" x-ref="pathInput"
                ...
                <div x-show="type === 'mysql' || type === 'postgres'" x-cloak class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    <x-field label="Database" for="database"><x-input id="database" name="database" :value="old('database', $src['database'] ?? '')" /></x-field>
                    <x-field label="DB User" for="db_user"><x-input id="db_user" name="db_user" :value="old('db_user', $src['user'] ?? '')" autocomplete="off" /></x-field>
                    <x-field label="DB Password" for="db_password" hint="Leave blank to keep."><x-input id="db_password" name="db_password" type="password" autocomplete="new-password" /></x-field>
                </div>
                ...
                <div x-show="type === 'composite'" x-cloak class="space-y-6">
                    ...
                    <div class="border-t border-slate-100 pt-5">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-xs text-slate-400">Leave a password blank to keep the stored one</span>
                        </div>
                        <div class="space-y-2">
                            <template x-for="(d, i) in cdbs" :key="i">
                                <div class="grid grid-cols-12 gap-2 items-center">
                                    ...
                                    <input type="password" :name="`composite_dbs[${i}][password]`" x-model="d.password" placeholder="keep" autocomplete="new-password" class="col-span-2 rounded-lg border-0 bg-white px-2.5 py-2 text-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-inset focus:ring-brand-500">
            ...
            <div x-show="tab==='schedule'" x-cloak x-data="{ cron: '{{ old('schedule_cron', $job->schedule_cron) }}' }">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <x-field label="Template" for="tmpl">
                    ...
                    <x-field label="Cron Expression" for="schedule_cron">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-5 mt-5">
                    <x-field label="Keep Latest" for="keep_latest"><x-input id="keep_latest" name="keep_latest" type="number" :value="old('keep_latest', $p?->keep_latest ?? 0)" min="0" /></x-field>
                    <x-field label="Keep Daily" for="keep_daily"><x-input id="keep_daily" name="keep_daily" type="number" :value="old('keep_daily', $p?->keep_daily ?? 7)" min="0" /></x-field>
